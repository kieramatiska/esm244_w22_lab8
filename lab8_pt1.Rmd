...
humboldt_wind <- as(humboldt_sp, "owin")

voles_full <- ppp(voles_ppp$x, voles_ppp$y, window = humboldt_wind)
...
gfunction <- envelope(voles_full, fun = Gest, # not a data frame
                      r = r_vec, nsin = 100,
                      nrank = 2)
...
lfunction_long <- lfunction %>% 
  as.data.frame() %>% 
  pivot_longer(cols = obs:hi, names_to = "model", 
               values_to = "l_val")
